...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER PRT_load_production_type_list");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT PRT_load_production_type_list");
#endif
  return ret_val;
}
...
void PRT_free_production_type_list ( GPtrArray *production_type_list )
{
  ...
  if ( production_type_list != NULL )
  {
    ...
    {
      p_data = (HRD_production_type_data_t*)(g_ptr_array_index (production_type_list, i ));
      if ( p_data != NULL )
      {
        g_free( p_data->name );
        g_free( p_data );
...
HRD_production_type_data_t *PRT_new_production_type_t( char *_name, int _id )
{
  HRD_production_type_data_t *ret_val = NULL;

  /*if ( ( _name != NULL ) && ( _id > 0 ) )*/
  {
/*    if ( strlen( _name ) > 0 )*/
    {
      if ( (ret_val = g_new (HRD_production_type_data_t, 1)) != NULL )
      {
...
void clear_production_type_list_data( GPtrArray *_production_type_list )
{
  ...
  HRD_production_type_data_t *p_data;
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- ENTER clear_production_type_list_data");
#endif
...
#if DEBUG
  g_log (G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "----- EXIT clear_production_type_list_data");
#endif
}
#endif
