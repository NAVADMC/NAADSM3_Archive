...
typedef struct
{
  ...
  GHashTable *detected; /**< A table for tracking detections of unique units.
    ...
    (HRD_herd_t *), and the associated value is irrelevant. */
                        ...
                        RAN_gen_t * rng)
{
  ...
  p = g_hash_table_lookup (local_data->detected, herd);
  if (p == NULL)
    {
      /* This unit has not been detected before in this simulation. */
      g_hash_table_insert (local_data->detected, herd, GINT_TO_POINTER(1));
