     ...
     lt_dlmutex_geterror *geterror;
{
  ...
  if ((lock && unlock && seterror && geterror) || !(lock || unlock || seterror || geterror))
    {
...
static NSSymbol (*ltdl_NSLookupSymbolInImage) (const struct mach_header * image,
                                               const char *symbolName, unsigned long options) = 0;
static enum DYLD_BOOL (*ltdl_NSIsSymbolNameDefinedInImage) (const struct mach_header * image,
              ...
              if (!mh1)
                {
                  ...
                  mh1 =
                    ltdl_NSAddImage ((char *) (((struct dylib_command *) lc)->dylib.name.offset +
                                               (unsigned long) lc),
  ...
  else
    {
      ...
      err =
        _dyld_func_lookup ("__dyld_NSIsSymbolNameDefinedInImage",
                           (unsigned long *) &ltdl_NSIsSymbolNameDefinedInImage);
      err =
...
static int foreach_dirinpath LT_PARAMS ((const char *search_path,
                                         ...
                                         foreach_callback_func * func, lt_ptr data1, lt_ptr data2));
...
static lt_dlhandle *find_handle LT_PARAMS ((const char *search_path,
                                            const char *base_name, lt_dlhandle * handle));
...
static int lt_argz_insert LT_PARAMS ((char **pargz,
                                      size_t * pargz_len, char *before, const char *entry));
    ...
    for (src = 0; path[src] != LT_EOS_CHAR; ++src)
      {
        ...
        if (path[src] == LT_PATHSEP_CHAR)
          {
            if ((dest == 0) || (path[1 + src] == LT_PATHSEP_CHAR) || (path[1 + src] == LT_EOS_CHAR))
              continue;
          }
        ...
        /* Directory separators are converted and copied only if they are
           ...
           NULL terminator.  */
        else if ((path[1 + src] != LT_PATHSEP_CHAR) && (path[1 + src] != LT_EOS_CHAR)
#ifdef LT_DIRSEP_CHAR
     ...
     const char *loader_name;
{
  ...
  if ((dlloader == 0)           /* diagnose null parameters */
      || (dlloader->module_open == 0) || (dlloader->module_close == 0) || (dlloader->find_sym == 0))
    {
